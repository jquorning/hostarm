...
package body CGI is
   ...
   function Field_End
     (Data        : Unbounded_String; Field_Separator : Character;
      Starting_At : Positive := 1) return Natural
   is
   ...
   procedure URL_Decode
     (Data : in out Unbounded_String; Translate_Plus : Boolean := True)
   is
      I                      : Positive := 1;
   ...
   function URL_Decode
     (Data : in Unbounded_String; Translate_Plus : Boolean := True)
   ...
   procedure URL_Encode
     (Data : in out Unbounded_String; Translate_Plus : Boolean := False)
   is
      I                 : Positive := 1;
   ...
   function URL_Encode
     (Data : in Unbounded_String; Translate_Plus : Boolean := False)
      return Unbounded_String
   is
      Destination : Unbounded_String := Data;
   begin
   ...
   begin
      while I <= Current_Length loop
         ...
         if Current_Character = '&' then
            ...
            --  This is problematic, because some very old browsers don't
            --  handle &quot; correctly.  However, we HAVE to do this,
            --  because otherwise attribute values will incorrectly
            --  terminate in odd places.  It _is_ in the standard, and
            --  current versions of Netscape Navigator, Microsoft Internet
            --  Explorer, and lynx all handle this correctly.
            Replace_Slice (Data, I, I, "&quot;");
   ...
   function HTML_Encode (Data : in Unbounded_String) return Unbounded_String is
      Destination : Unbounded_String := Data;
   begin
   ...
   function Value
     (Key      : in Unbounded_String; Index : in Positive := 1;
   ...
   function Value
     (Key      : in String; Index : in Positive := 1;
   ...
   function Key_Exists
     (Key : in Unbounded_String; Index : in Positive := 1) return Boolean
   is
      My_Index : Positive := 1;
   begin
      for I in 1 .. Argument_Count loop
   ...
   function Key_Exists
     (Key : in String; Index : in Positive := 1) return Boolean
   is
   begin
   ...
   function Key_Value_Exists
     (Key : in Unbounded_String; Value : in Unbounded_String) return Boolean
   is
   begin
      for I in 1 .. Argument_Count loop
         if CGI_Data.all (I).Key = Key and then CGI_Data.all (I).Value = Value
         then
            return True;
         end if;
      end loop;
   ...
   function Key_Value_Exists
     (Key : in String; Value : in String) return Boolean
   is
   begin
      return
        Key_Value_Exists
   ...
   begin
      return
        "http://" & Get_Environment ("SERVER_NAME") &
...
is
   ...
   procedure Set_CGI_Position
     (Key_Number : in Positive; Datum : in Unbounded_String)
   is
   ...
   begin
      return
        To_String (Cookie_Value (To_Unbounded_String (Key), Index, Required));
   end Cookie_Value;
   ...
   procedure Set_Cookie
     (Key    : String; Value : String; Expires : String := "";
   ...
   procedure Initialize is
      Raw_Data            :
        Unbounded_String;  -- Initially an empty string (LRM A.4.5(73))
      Request_Method_Text : constant String :=
   ...
   begin
      if Request_Method_Text = "GET" then
         ...
         declare
            Raw_Data_String :
              String (1 .. Integer'Value (Get_Environment ("CONTENT_LENGTH")));
         begin
            Get (Raw_Data_String);
      ...
      Translate
        (Raw_Data, Mapping => Plus_To_Space); -- Convert "+"s to spaces.
